# E:/MRVNet2D/test_alltasks_complex_mrvnet.py
...
# ---------------- Distortion Pipeline Import ----------------
from multiple_distortion.utils_data import distort_images

DEVICE = torch.device("cuda" if torch.cuda.is_available() else "cpu")

# ✅ 체크포인트 경로 (MRVNet 전용)
CKPT = r"E:\MRVNet2D\checkpoints\train_all_task\epoch_77_ssim0.8999_psnr31.55.pth"
...
# ✅ 저장 디렉토리
DISTORTION_DIR = r"E:/MRVNet2D/distortion_imgs/distortion"
RESULTS_DIR = r"E:/MRVNet2D/distortion_imgs/results"
...
def main():
    ...
    for clean_dir in CLEAN_DIRS:
        ...
        img_paths = [
            p
            for p in img_paths
            if os.path.splitext(p)[1].lower() in [".png", ".jpg", ".jpeg", ".bmp", ".tif"]
        ]
        ...
        for idx, path in enumerate(tqdm(img_paths, desc=os.path.basename(clean_dir))):
            ...
            # ✅ 복합 왜곡 생성 (1~7개 distortion 랜덤 조합)
            distorted, funcs, vals = distort_images(
                gt_tensor.squeeze(0).clone(),
                max_distortions=7,  # 최대 7개 왜곡
                num_levels=5
            )
            ...
            # 로그 출력
            print(
                f"[{img_counter}] {os.path.basename(path)} | "
